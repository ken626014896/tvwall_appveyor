...
public:
    ...
    Q_PROPERTY(bool iszhongwen READ get_iszhongwen WRITE set_iszhongwen NOTIFY iszhongwen_changed)

    Q_INVOKABLE bool get_iszhongwen();
    Q_INVOKABLE void set_iszhongwen(bool val);
    ...
    void check_permission(const QString &permission);  //安卓动态申请授权
...
//    Q_INVOKABLE void add_device_display(QString videoId, double width, double height, double x, double y, int wall_width, int wall_height);
    ...
    Q_INVOKABLE void setIP(QString, QString, QString); //单场景登录
    Q_INVOKABLE void login_multiple();                 //多场景登录
...
//    Q_INVOKABLE void set_aduio_param();
    ...
    Q_INVOKABLE void init_language(); //初始化语言
    ...
    Q_INVOKABLE int which_os();//查找信号源

    Q_INVOKABLE void get_current_dir();//查找信号源
    QString get_dev_cmdType(QHash<int, QSharedPointer<acc_conf_info>> m_hash,int dev_type);

    QSharedPointer<acc_conf_info> get_diff_dev(QHash<int, QSharedPointer<acc_conf_info>> m_hash,int sort,QString model);
    void match_dev(QSharedPointer<data_info> val);

    //获取当前场景实际大屏的宽和高
    Q_INVOKABLE void set_wall_width_height(QString w,QString h);
signals:
    void index_room_changed();
    void iszhongwen_changed();
    ...
    void sence_changed();
    void login_error();//登录失败提醒
    void login_success();//登录失败提醒
    void login_msg(int hint);//登录失败提醒
    void ip_error();
    void config_error();
    //二期新增
    ...
    void test_signal22(double x, double y, double width, double height, unsigned int win_id);
...
//    QQmlApplicationEngine *m_engine;
    ...
    int get_dev_portRcv(QList<QSharedPointer<data_info> > list,QString ip_adress,int portSend); //获取udp设备的接受返回信息的端口
