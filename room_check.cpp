...
room_check::room_check(QObject *parent) : QObject(parent),
  ...
  ,isConfig(1),tiemr_conf(nullptr),current_index(0),len(0),first_can_login(-1)
{
...
//    connect(m_tcp_socket,&QTcpSocket::stateChanged,this,&room_check::onSocketStateChange);
    ...
    tiemr=new QTimer();
    tiemr_conf=new QTimer();
    connect(tiemr,&QTimer::timeout,this,&room_check::time_out);
    connect(tiemr_conf,&QTimer::timeout,this,&room_check::time_out_conf);
}
...
void room_check::start_check_conf(QList<QSharedPointer<data_info>> val) //开始逐个检查场景是否可用,检查到可用的就使用这个登录
{
    if(val.length()==0)
        return;
    isConfig=2;
    ...
    vall=val;
//    qDebug()<<len;
    start_check_conf_one(vall.at(0));

}

void room_check::start_check_conf_one(QSharedPointer<data_info> room)
{
    first_can_login=current_index;
    usr=room->get_user_name();
    pwd=room->get_user_pwd();
    IP=room->get_ip_address();
    PORT=QString::number(room->get_port());
    qDebugxxxx(qtr("检查该多场景中该场景是否可用"), room->get_name(),room->get_ip_address(), QString::number(room->get_port()));
    qDebug()<<"begain check"<<room->get_name();
    m_tcp_socket->connectToHost(IP, (quint16)PORT.toInt());
    tiemr_conf->start(3000);
...
int room_check::get_currentindex()
{
    return first_can_login;
}

void room_check::coni_check_conf_one(int index)
{
    if(index>=len){

        emit login_status_conf(-1);
    }
    else {
        first_can_login=index;
        qDebug()<<"continue"<<index;
        IP=vall.at(index)->get_ip_address();
        PORT=QString::number(vall.at(index)->get_port());
...
void room_check::connected()
{
    ...
    if(isConfig==1){
        ...
        if(res.count() <= 0) {
            qWarnx(qtr("检查连接，登录时失败")); //连接成功但登录失败
    //        reconnected();
...
//        emit login_status(true,IP, PORT, usr, pwd);
        ...
        emit login_status_conf(first_can_login);
        m_tcp_socket->abort();
        stop();
    }
...
//           stop();
    ...
    case QAbstractSocket::HostLookupState:
        qDebug()<<(QStringLiteral("check scoket状态：HostLookupState"));
        break;
    case QAbstractSocket::ConnectingState:
        qDebug()<<(QStringLiteral("check scoket状态：ConnectingState"));
        break;

    case QAbstractSocket::ConnectedState:
        qDebug()<<(QStringLiteral("check scoket状态：ConnectedState"));
        break;

    case QAbstractSocket::BoundState:
        qDebug()<<(QStringLiteral("check scoket状态：BoundState"));
        break;

    case QAbstractSocket::ClosingState:
        qDebug()<<(QStringLiteral("check scoket状态：ClosingState"));
        break;

    case QAbstractSocket::ListeningState:
        qDebug()<<(QStringLiteral("check scoket状态：ListeningState"));
    }
}
...
//    qDebug()<<current_index;
    ...
    qDebug()<<"time_login_error";

    start_check_conf_one(vall.at(current_index));
...
QByteArray room_check::get_response2()
{
    if(!m_tcp_socket->waitForReadyRead(2000)) {
        qWarnx(qtr("未接收到响应"));
        return QByteArray();
    }
    int len = 0;
